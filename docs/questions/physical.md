...
Congestion occurs when too many wires need routing in a limited area. It leads to:
- **Routing failures**
- **Higher delay and timing violations**
- **Increased power consumption**

**Congestion resolution techniques:**
...
OCV variations are mitigated using **derating factors** and **advanced techniques like AOCV & POCV**.
...
- **Lower metal layers (M1-M3)**: Local interconnects (high resistance).
...
**IR drop** occurs when there is voltage loss due to resistance in power delivery. It affects **timing and functionality**.

- **Static IR Drop**: Average power consumption.
- **Dynamic IR Drop**: Transient power fluctuations.

Mitigated by **power grid optimization, decoupling capacitors, and thick metal layers**.
...
**Fixes:**
- **Diode insertion** at sensitive gates.
- **Breaking long nets** using vias.
...
Congestion causes **longer wire detours**, increasing:
- **Wire resistance (R) and capacitance (C)**
- **Signal delay**
- **Setup violations**

**Fix**: Use congestion-aware placement and routing techniques.
...
- **IR Drop Analysis**: Checks power integrity.
- **DRC/LVS Checks**: Ensures design rule compliance.
- **Antenna Rule Check**: Prevents device damage.
